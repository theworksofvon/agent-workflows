...
import { githubPoller, type GitHubPollingClient } from "../src/github/poller.js";
...
test("github poller skips existing comments on a new installation", async () => {
  ...
  try {
    ...
    const client = {
      ...
      async listReviewComments() {
        return [{
          ...
          originalLine: 1,
          diffHunk: "@@",
          createdAt: new Date(includeNewComment ? 2_000 : 1_000).toISOString(),
          reviewId: null,
        }];
      },
    } satisfies GitHubPollingClient;
    ...
    assert.deepEqual(events.map((event) => event.payload.comments[0].id), [11, 21]);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("github poller filters self, marker, bot, cursor, and processed comments and groups reviews", async () => {
  ...
  try {
    ...
    const client = {
      async listOpenPRs() {
        return [{ number: 1, title: "Ready", body: "body", headRef: "head", baseRef: "main", draft: false }];
      },
      async listIssueComments() {
        return [
          { id: 1, author: "reviewer", body: "<!-- agent-workflows:bot --> own", createdAt: "2020-01-01T00:00:00Z" },
          { id: 2, author: "agentuser", body: "own", createdAt: "2020-01-01T00:00:01Z" },
          { id: 3, author: "dependabot[bot]", body: "bot", createdAt: "2020-01-01T00:00:02Z" },
          { id: 4, author: "human", body: "conversation", createdAt: "2020-01-01T00:00:03Z" },
        ];
      },
      async listReviewComments() {
        return [
          { id: 10, author: "human", body: "<!-- agent-workflows:bot -->", path: "a.ts", line: null, originalLine: null, diffHunk: "@@", createdAt: "2020-01-01T00:00:04Z", reviewId: null },
          { id: 11, author: "AGENTUSER", body: "own", path: "a.ts", line: null, originalLine: 4, diffHunk: "@@", createdAt: "2020-01-01T00:00:05Z", reviewId: null },
          { id: 12, author: "human", body: "ungrouped", path: "a.ts", line: null, originalLine: 4, diffHunk: "@@", createdAt: "2020-01-01T00:00:06Z", reviewId: null },
          { id: 13, author: "human", body: "grouped", path: "b.ts", line: 5, originalLine: 4, diffHunk: "@@", createdAt: "2020-01-01T00:00:07Z", reviewId: 99 },
        ];
      },
    } satisfies GitHubPollingClient;
    const poller = githubPoller({ config, client });
    ...
    assert.deepEqual(payloads.map((payload) => payload.comments[0].id), [4, 12, 13]);
...
test("github poller isolates repository failures and continues polling healthy repositories", async () => {
  ...
  try {
    const config = makeConfig(root);
    config.repos = [{ owner: "bad", repo: "repo" }, { owner: "good", repo: "repo" }];
    const client = {
...
test("github poller ignores processed issue and review keys even beyond their cursors", async () => {
  ...
  try {
    ...
    state.markBatchCompleted({
      repo: config.repos[0], prNumber: 1, prTitle: "PR", prBody: null, headRef: "head", baseRef: "main",
      batchId: "processed", groupKey: "processed", firstSeenAt: "now", lastSeenAt: "now", attempts: 1,
      comments: [
        { key: "local-owner/sample-repo#1:issue:100", id: 100, kind: "issue", author: "human", body: "done", createdAt: "now" },
        { key: "local-owner/sample-repo#1:review:200", id: 200, kind: "review", author: "human", body: "done", createdAt: "now" },
      ],
    });
    const client = {
      async listOpenPRs() { return [{ number: 1, title: "PR", body: null, headRef: "head", baseRef: "main", draft: false }]; },
      async listIssueComments() { return [{ id: 100, author: "human", body: "done", createdAt: "now" }]; },
      async listReviewComments() { return [{ id: 200, author: "human", body: "done", path: "a.ts", line: null, originalLine: null, diffHunk: "@@", createdAt: "now", reviewId: null }]; },
    } satisfies GitHubPollingClient;
    assert.deepEqual(await githubPoller({ config, client }).poll(), []);
  } finally {
    rmSync(root, { recursive: true, force: true });
