...
const repos = (env.REPOS ?? "").split(",").map((value) => value.trim()).filter(Boolean);
check(
...
check(allowedAgents.has(primaryAgent), `AGENT=${primaryAgent}`, `Unsupported AGENT=${primaryAgent}`);
...
function checkAgent(agent) {
  const specs = {
    codex: { envName: "CODEX_BIN", fallback: "codex", auth: ["login", "status"] },
    "claude-code": { envName: "CLAUDE_CODE_BIN", fallback: "claude", auth: ["auth", "status"] },
  ...
  if (!spec.auth) return;
  const result = spawnSync(resolved, spec.auth, { encoding: "utf8", timeout: 10_000 });
  const output = `${result.stdout ?? ""}\n${result.stderr ?? ""}`;
  const authenticated = result.status === 0 && !/loggedIn"?:\s*false|not logged|authentication required/i.test(output);
  check(authenticated, `${agent} authentication is available`, `${agent} is installed but not authenticated`);
}
...
function checkCommand(command, args, label) {
  ...
  const result = spawnSync(resolved, args, { encoding: "utf8", timeout: 10_000 });
  check(result.status === 0, `${label}: ${(result.stdout || result.stderr).trim()}`, `${label} failed to run`);
}
